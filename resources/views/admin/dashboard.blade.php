                ...
                @foreach ($admins as $admin)
                    <div class="admin-card">
                        ...
                        <div class="action-buttons">
                            <button class="edit"
                                onclick="openEditModal({{ $admin->id }}, '{{ $admin->name }}', '{{ $admin->email }}')">
                                <i class="bi bi-pencil-square"></i>
                            </button>
                            <form id="delete-form-{{ $admin->id }}" action="{{ route('admin.destroy', $admin->id) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
            ...
            <div class="otp-grid">
                @foreach($otps as $otp)
                    <div class="otp-card">
                        ...
                        <span
                            class="badge {{ $otp->status === 'used' ? 'badge-used' : 'badge-unused' }}">{{ ucfirst($otp->status) }}</span>
                        <p>Digunakan oleh: {{ $otp->user?->name ?? '-' }} {{ $otp->user?->email ?? '' }}</p>
                        <form action="{{ route('admin.otp.destroy', $otp->id) }}" method="POST"
                            onsubmit="return confirm('Yakin hapus OTP?')">
                            @csrf
                            @method('DELETE')
    ...
    <div id="modal" class="modal">
        <div class="modal-content">
            ...
            <form action="{{ route('pengumuman.store') }}" method="POST">
                ...
                <button type="submit"
                    style="background:#3b82f6;color:#fff;padding:6px 10px;border-radius:5px;">Buat</button>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editAdminForm" method="POST">
                ...
                <button type="submit"
                    style="background:#10b981;color:#fff;padding:6px 10px;border-radius:5px;">Simpan</button>
            </form>
        </div>
    </div>
