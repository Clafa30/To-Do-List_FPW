    ...
    <div class="login__content">
      <div class="login__img">
        <lottie-player src="https://assets10.lottiefiles.com/packages/lf20_jcikwtux.json" background="transparent"
          speed="1" style="width:100%; max-width:380px; height:380px;" loop autoplay>
        </lottie-player>
      </div>
      ...
      <div class="login__forms form-wrapper">
        ...
        <form action="{{ route('login') }}" method="POST"
          class="login__form form-container {{ ($activeForm ?? 'login') == 'login' ? 'active' : '' }}" id="login-in"
          autocomplete="off">
          @csrf
          <h1 class="login__title">Sign In</h1>
          @include('components.alert')

          {{-- Dummy hidden inputs untuk cegah autofill --}}
          <input type="text" name="fakeusernameremembered" style="display:none">
          <input type="password" name="fakepasswordremembered" style="display:none">

          <div class="login__box">
            <i class="bx bx-at login__icon"></i>
            <input type="email" name="email" placeholder="Email" class="login__input" autocomplete="off" required />
          </div>
        ...
        <form action="{{ route('register') }}" method="POST"
          class="login__form form-container {{ ($activeForm ?? 'login') == 'register' ? 'active' : '' }}" id="login-up"
          ...
          <div class="login__box">
            ...
            <input type="text" name="name" placeholder="Full Name" class="login__input" autocomplete="off" required />
          </div>
